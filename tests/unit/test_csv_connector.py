...
# @MX:ANCHOR: CSV test suite entry point (fan_in >= 3: CI, test runner, coverage)
# @MX:REASON: Comprehensive test coverage for CSV connector functionality
# @MX:SPEC: SPEC-PLATFORM-001 TDD methodology requirement
...
def large_csv_file() -> tempfile._TemporaryFileWrapper:
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".csv", delete=False) as f:
        ...
        for i in range(10000):  # 10k rows for testing
            writer.writerow(
                [i, f"Item_{i}", i * 100.5, f"2023-01-{i % 28 + 1}", f"Cat_{i % 5}"]
            )
        temp_path = f.name
        f.flush()
...
class TestCSVConnectorTypeInference:
    ...
    def test_infer_boolean_type(self, sample_csv_data):
        ...
        connector = CSVConnector()
        inferred_type = connector._infer_type(
    ...
    def test_import_csv_to_duckdb_table(self, sample_csv_data, mock_database):
        ...
        try:
            connector.import_to_duckdb(
                csv_path=temp_path, db_connection=mock_database, table_name="users"
            )
    ...
    def test_register_custom_connector(self):
        ...
        try:
            ...
            class CustomConnector(BaseConnector):
                ...
                def read(self, source: str, **kwargs):
                    _ = source  # Unused in mock
                    yield {}

                def validate(self, source: str, **kwargs):
                    _ = source, kwargs  # Unused in mock
                    return True

                def get_metadata(self, source: str, **kwargs):
                    _ = source, kwargs  # Unused in mock
                    return {}
